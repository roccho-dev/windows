param(
    [Parameter(Mandatory)]
    [ValidateSet('Plan', 'Init', 'Clone', 'Tools', 'Run')]
    [string] $Step,
...
# exit 4: /nix is not the store Init seeded from this image.
...
# Clone, Tools and Run run only on the volume Init seeded; the image is their first argument.
...
# Runtime = instantiate(Spec, Binding) as exact wslc argv.
$Plan = [ordered]@{
    site = $Site.site
    wslc = $Wslc
    Init = [ordered]@{
